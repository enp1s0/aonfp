...
int main() {
	...
	std::uniform_real_distribution<DEVICE_T> dist(-1.0, 1.0);
	...
	for (std::size_t i = 0; i < N; i++) {
		aonfp::q::decompose(src_s_exp_array[i], src_mantissa_array[i], dist(mt));
	}

	{
		cudaDeviceSynchronize();
		const auto start_clock = std::chrono::system_clock::now();
		aonfp::q::cuda::copy_to_device(device_array, 1, src_s_exp_array, 1, src_mantissa_array, 1, N);
		cudaDeviceSynchronize();
...

	{
		cudaDeviceSynchronize();
		const auto start_clock = std::chrono::system_clock::now();
		aonfp::q::cuda::copy_to_host(dst_s_exp_array, 1, dst_mantissa_array, 1, device_array, 1, N);
		cudaDeviceSynchronize();
		const auto end_clock = std::chrono::system_clock::now();
	...
	double max_error = 0;
	for (std::size_t i = 0; i < N; i++) {
		const auto src = aonfp::q::compose<double>(src_s_exp_array[i], src_mantissa_array[i]);
		const auto dst = aonfp::q::compose<double>(dst_s_exp_array[i], dst_mantissa_array[i]);
