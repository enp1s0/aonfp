...
int main() {
...

	{
		...
		aonfp::q::cuda::copy_to_device(device_array, src_s_exp_array, src_mantissa_array, N);
		cudaDeviceSynchronize();
...

	{
		cudaDeviceSynchronize();
		const auto start_clock = std::chrono::system_clock::now();
		aonfp::q::cuda::copy_to_host(dst_s_exp_array, dst_mantissa_array, device_array, N);
		cudaDeviceSynchronize();
		const auto end_clock = std::chrono::system_clock::now();
